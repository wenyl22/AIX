...
class Mesh_longrange_HiRy(SimpleTopology):
    description = "Mesh_longrange_HiRy"
    ...
    def getWeight(self, dirn):
        res = []
        if dirn == "SameWest" or dirn[:5] == "North":
            res.append(1)
        else: res.append(2)
        if dirn == "SouthSame" or dirn[-4:] == "West":
            res.append(3)
        else: res.append(4)
        return res
    ...
    def makeTopology(self, options, network, IntLink, ExtLink, Router):
        ...
        # VC2:
        # P_3 = {{X^-, Y^-}, {X^-, Y^0}, {X^-, Y^+}, {X^0, Y^-}}
        # P_4 = {{X^+, Y^-}, {X^+, Y^0}, {X^+, Y^+}, {X^0, Y^+}}
        ...
        for (i, j) in Links:
            ...
            int_links.append(
                IntLink(
                    ...
                    dst_inport=rev_dirn,
                    latency=link_latency,
                    per_vc_weight=self.getWeight(dirn),
                )
            )
            ...
            int_links.append(
                IntLink(
                    ...
                    dst_inport=dirn,
                    latency=link_latency,
                    per_vc_weight=self.getWeight(rev_dirn),
                )
            )
            link_count += 2
